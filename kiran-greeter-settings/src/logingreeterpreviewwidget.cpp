...
void LoginGreeterPreviewWidget::paintEvent (QPaintEvent *event)
{
    ...
    ///绘制输入框
    painterPath.addRect((DEFAULT_SIZE.width() - EDIT_SIZE.width()) / 2,
                        (DEFAULT_SIZE.height() - EDIT_SIZE.height()) / 2,
                        EDIT_SIZE.width(), EDIT_SIZE.height());

    ///绘制时间框
    painterPath.addRect((DEFAULT_SIZE.width() - TIME_LABEL_SIZE.width()) / 2,
                        DEFAULT_SIZE.height() - TIME_LABEL_BOTTOM_SPACE - TIME_LABEL_SIZE.height(),
                        TIME_LABEL_SIZE.width(), TIME_LABEL_SIZE.height());

    ///绘制用户列表
    QRect user5(USER_LIST_BOTTOM_RIGHT_SPACE,
                DEFAULT_SIZE.height() - USER_LIST_BOTTOM_RIGHT_SPACE - USER_LIST_ITEM_SIZE.height(),
    ...
    ///绘制按钮
    QRect button3(DEFAULT_SIZE.width() - BUTTON_RIGHT_SPACING - BUTTON_SIZE.width(),
                  DEFAULT_SIZE.height() - BUTTON_SIZE.height() - BUTTON_SPACING,
