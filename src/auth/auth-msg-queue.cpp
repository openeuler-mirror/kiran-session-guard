...
AuthMsgQueue::AuthMsgQueue(QObject *parent)
    : AuthMsgQueueBase(parent),
      ...
      m_dispatcherWorker(new DispatcherWorker(this))
{
...
void AuthMsgQueue::stopDispatcher()
{
    if(m_dispatcherThread->isRunning())
    {
        //TODO:取消掉DispatcherWorker::doDispatcher里的阻塞
...
void AuthMsgQueue::appendAuthCompleteMsg(bool authRes,const QString &msg)
{
    m_dispatcherWorker->appendAuthCompleteMsg(authRes,msg);
}
...
void DispatcherWorker::appendAuthCompleteMsg(bool authRes,const QString &msg)
{
    AuthMsg authMsg;
...
void DispatcherWorker::doDispatcher()
{
    forever
    {
        ...
        case AUTH_MSG_TYPE_COMPLETE:
            emit authenticationComplete(msg.extra.complete_.authRes,msg.msg);
            break;
        }
    }
}
