...
class AuthProxy : public QObject
{
    Q_OBJECT
public:
    Q_ENUMS(PromptType MessageType AuthType)
    explicit AuthProxy(QObject* parent = nullptr);
    ~AuthProxy();
...
signals:
    ...
    /// 认证完成结果
    void authenticationComplete(bool authRes, QString text);
...
private slots:
    ...
    void handleAuthQueueComplete(bool authRes, QString msg);
