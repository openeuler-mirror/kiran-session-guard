...
void PamAuthProxy::startAuthenticate(const QString &userName)
{
    ...
    {
        LOG_WARNING_S() << "start failed,is running";
        return;
    }
...
void PamAuthProxy::reAuthenticate()
{
    ...
    {
        LOG_WARNING_S() << "current state is not wait reauth";
        return;
    }
    m_reAuthCOndition.wakeAll();
}
...
void PamAuthProxy::response(const QString &str)
{
    ...
    {
        LOG_WARNING_S() << "current state is nor wait response";
        return;
    }
...
int PamAuthProxy::conversation(int num_msg, const pam_message **msg, pam_response **resp, void *appdata_ptr)
{
    ...
    for (replies = 0; replies < num_msg && ret == PAM_SUCCESS; replies++)
    {
        ...
        {
            LOG_INFO("thread interruption is set,skip msg [%s]", msg[replies]->msg ? msg[replies]->msg : "null");
            goto next_msg;
        }
        LOG_INFO_S() << "conversation type[" << msg[replies]->msg_style << "]"
                     << " " << msg[replies]->msg;
...
void PamAuthProxy::run()
{
    ...
    LOG_INFO_S() << "pam_start-->" << m_authUserName;
    ...
    LOG_INFO_S() << "pam auth thread finished";
}
