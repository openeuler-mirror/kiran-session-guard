...
GreeterSetting::GreeterSetting ()
        : m_backgroundPath(DEFAULT_BACKGROUND_URI), m_userlistHiding(DEFAULT_USER_LIST_HIDING),
          m_enableManualLogin(DEFAULT_ENABLE_MANUAL_LOGIN), m_enableScaling(SCALING_AUTO),
          m_scaleFactor(DEFAULT_SCALE_FACTOR), m_messageDisplayInterval(DEFAULT_MESSAGE_INTERVAL)
{
    load();
}
